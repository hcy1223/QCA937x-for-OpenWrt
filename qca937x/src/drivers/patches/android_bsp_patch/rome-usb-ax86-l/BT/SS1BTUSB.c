...
#define INTR_READ_SIZE                  512    /* Size of Read Buffer that   */
                                                /* is submitted to Intr Read */
                                                /* endpoint (in URB).        */
...
/* This function frees all the write information structures and memory  */
static void FreeDeviceWriteInformationEntry(USBBufferInformation_t *EntryToFree)
{
   int Index;
...
static int InitializeInterface1(USBDeviceInformation_t *USBDeviceInformationPtr)
{
   ...
   /* This function copy's the URB memory to the circlur buffers in the */
   /* ReadInformation parameter that is passed in. The first parameter  */
   /* is a pointer to the URBPtr that stores the memory to be copied    */
   /* over. The second parameter is a pointer to the USBBufferInformation */
   /* that store's the memory to be copied to.                          */
static void CopyURBMemoryToLocalReadMemory(struct urb *URBPtr, USBBufferInformation_t *BufferInfo, unsigned char PktType)
{
   int            LengthOfData;
   ...
   if(URBPtr)
   {
      ...
      if(((LengthOfData + BTUSB_PKT_IND_LENGTH) <= (BufferInfo->CircleBufferSize - BufferInfo->CircleBufferBytesAvailable)))
      {
         BufferInfo->CircleBuffer[BufferInfo->CircleBufferWriteIndex] = PktType;
	 BufferInfo->CircleBufferWriteIndex += BTUSB_PKT_IND_LENGTH;
         ...
         BufferInfo->CircleBufferBytesAvailable += LengthOfData + BTUSB_PKT_IND_LENGTH;
...
#endif
{
   ...
   if(URBPtr)
   {
      ...
      if(USBDeviceInformationPtr)
      {
         //printk(KERN_ERR "%s : received %d bytes\n", __FUNCTION__, URBPtr->actual_length);
         ...
         if((!URBPtr->status) || (URBPtr->status == -ECOMM))
         {
            if(URBPtr->status == -ECOMM)
            {
               ...
               /* system memory.                                        */
               printk(KERN_ERR "%s : Possible data loss\n", __FUNCTION__);
            }
            ...
            {
               //printk(KERN_ERR "%s : HCI Event %d bytes\n", __FUNCTION__, URBPtr->actual_length);

               /* Copy over the URB information into our circlur buffer */
               /* so the user can read the information back.            */
               CopyURBMemoryToLocalReadMemory(URBPtr, &USBDeviceInformationPtr->HCIBufferInformation, BTUSB_PACKET_TYPE_EVENT);
            ...
            else
            {
               ...
               {
                  printk(KERN_ERR "%s : ACL Data %d bytes\n", __FUNCTION__, URBPtr->actual_length);

                  /* Copy over the URB information into our circular    */
                  /* buffer so the user can read the information back.  */
                  CopyURBMemoryToLocalReadMemory(URBPtr, &USBDeviceInformationPtr->ACLBufferInformation, BTUSB_PACKET_TYPE_ACL);
                  ...
                  /* information.                                       */
                  usb_fill_bulk_urb(USBDeviceInformationPtr->ACLBufferInformation.ReadURBHandle, USBDeviceInformationPtr->USBDevice, pipe, USBDeviceInformationPtr->ACLBufferInformation.ReadBuffer, USBDeviceInformationPtr->ACLBufferInformation.ReadBufferSize, USBReadCallback, (void *)DevNum);
               ...
               else
               {
                  ...
                  //printk(KERN_ERR "%s : SCO Data %d bytes\n", __FUNCTION__, URBPtr->actual_length);
               ...
               switch(URBPtr->status)
               {
                  ...
                  case -EPROTO :
                     ...
                     if(URBPtr == USBDeviceInformationPtr->HCIBufferInformation.ReadURBHandle)
                     {
                        ...
                        printk(KERN_ERR "%s : Resubmitted interrupt urb (%d)\n", __FUNCTION__, ret_val);
                     }
                     break;
                  case -EILSEQ :
                  case -EOVERFLOW :
                     ...
                     /* with the device.                                */
                     printk(KERN_ERR "%s : Possible hardware issue, URB status %d\n", __FUNCTION__, URBPtr->status);
...
#endif
{
   ...
   if((URBPtr) && (URBPtr->context))
   {
      ...
      {
         usb_autopm_put_interface(USBBufferInformationPtr->USBDeviceInformation->AsyncInterfacePtr);
...
static int USBProbe(struct usb_interface *InterfacePtr, const struct usb_device_id *DeviceID)
{
   ...
   if((USBDevice != NULL) && ((InterfaceNum == BLUETOOTH_USB_INTERFACE_0) || (InterfaceNum == BLUETOOTH_USB_INTERFACE_1)))
   {
      ...
      {
        printk(KERN_ERR "%s : Creating a new USBDevice\n", __FUNCTION__);
         ...
         if((USBDeviceInformation = (USBDeviceInformation_t *)kmalloc(sizeof(USBDeviceInformation_t), GFP_ATOMIC)) != NULL)
         {
            ...
            if(AddDeviceInformationEntry(&ContextInformation.DeviceInformationList, USBDeviceInformation))
            {
               ...
               if(!InitializeWriteInformation(&USBDeviceInformation->HCIBufferInformation, USB_ENDPOINT_XFER_CONTROL, 8))
               {
                  ...
                  /* interface.                                         */
                  printk(KERN_ERR "%s : couldn't initialize HCI command pipe\n", __FUNCTION__);
            ...
            else
            {
               ...
               printk(KERN_ERR "%s : Error adding new USBDevice\n", __FUNCTION__);
            }
         }
         else
         {
            ...
            printk(KERN_ERR "%s : Error allocating memory for new USBDevice\n", __FUNCTION__);
         }
      }
      else
      {
      ...
      if(USBDeviceInformation)
      {
         ...
         switch(InterfaceNum)
         {
            ...
            case BLUETOOTH_USB_INTERFACE_1:
               ...
               printk(KERN_ERR "%s : Found Interface 1\n", __FUNCTION__);
               ...
               {
                  printk(KERN_ERR "%s : Error : Interface Already Owned.", __FUNCTION__);
               }
               break;
            default:
               printk(KERN_ERR "%s : Not Processing Interface %d\n", __FUNCTION__, InterfaceNum);
               break;
         }
      }
      ...
      {
         printk(KERN_ERR "%s : We have a new device\n", __FUNCTION__);

         spin_lock_init(&(USBDeviceInformation->ReadInformationSpinLock));

         printk(KERN_ERR "%s : ReadInformation SpinLock Created\n", __FUNCTION__);

         init_waitqueue_head(&(USBDeviceInformation->ReadWaitEvent));

         printk(KERN_ERR "%s : ReadWait Event Created\n", __FUNCTION__);

         /* Initialize the work struct for recovering an endpoint from  */
         /* STALL status.                                               */
         INIT_WORK(&(USBDeviceInformation->ClearStallWork), ClearStall);

          USBDeviceInformation->MiscDev.minor           =       MISC_DYNAMIC_MINOR;
          USBDeviceInformation->MiscDev.name            =       "SS1BTUSB0";
          USBDeviceInformation->MiscDev.nodename        =       SS1BTUSB_CLASS_NAME;
          USBDeviceInformation->MiscDev.fops            =       (&(ContextInformation.FileOperations));

         /* We can register the device now, as it is ready.             */
         retval = misc_register(&(USBDeviceInformation->MiscDev));
         printk(KERN_ERR "%s: usb_register_dev return value: %d\n", __FUNCTION__, retval);
         if(retval)
         {
            printk(KERN_ERR "%s: usb_register_dev failed %d\n", __FUNCTION__, retval);
         ...
         {
            InterfacePtr->needs_remote_wakeup = 1;
            usb_enable_autosuspend(USBDeviceInformation->USBDevice);
            /* Bump reference count.                                    */
            try_module_get(THIS_MODULE);

            printk(KERN_ERR "%s : registered device %s interface->minor %d\n", __FUNCTION__, USBDeviceInformation->MiscDev.name, USBDeviceInformation->MiscDev.minor);

            /* Save minor device number to easy mapping later.          */
            USBDeviceInformation->MinorDeviceNumber = USBDeviceInformation->MiscDev.minor;
         }
      }
   }
...
static int USBSuspend(struct usb_interface *InterfacePtr, pm_message_t message)
{
   struct usb_device *USBDevice;
   unsigned int InterfaceNum;
   USBDeviceInformation_t *USBDeviceInformationPtr;
   ...
   if (PMSG_IS_AUTO(message) && InterfaceNum == BLUETOOTH_USB_INTERFACE_0)
      StopHCIEventACL(USBDeviceInformationPtr);
   //TODO: SCO needs to be taken care

   return 0;
}

static int USBResume(struct usb_interface *InterfacePtr)
{
   struct usb_device *USBDevice;
   unsigned int InterfaceNum;
   USBDeviceInformation_t *USBDeviceInformationPtr;
   ...
   if (USBDeviceInformationPtr && InterfaceNum == BLUETOOTH_USB_INTERFACE_0)
      StartHCIEventACL(USBDeviceInformationPtr, USBDeviceInformationPtr->MinorDeviceNumber);
   //TODO: SCO needs to be taken care
...
static void USBDisconnect(struct usb_interface *InterfacePtr)
{
   ...
   if(USBDeviceInformationPtr)
   {
      ...
      if(!USBDeviceInformationPtr->NumInterfacesOwned)
      {
         ...
         /* Give back our minor.                                        */
         misc_deregister(&(USBDeviceInformationPtr->MiscDev));
...
static int USBOpen(struct inode *INode, struct file *FilePointer)
{
   ...
   printk(KERN_ERR "%s : Enter, Inode %p FilePointer %p\n", __FUNCTION__, INode, FilePointer);
   ...
   if((INode) && (FilePointer))
   {
      ...
      if((USBDeviceInformationPtr) && (!USBDeviceInformationPtr->BeingRemoved))
      {
         ...
         if(!USBDeviceInformationPtr->OpenedByUser)
         {
...
#if 0
            BT_ERR("\n=========================================\n");
            BT_ERR("\t\tDownloading BT Firmware...");
            if (USBDeviceInformationPtr->USBDevice != NULL) {
	            rome_download(USBDeviceInformationPtr->USBDevice);
        	    BT_ERR("\t\tFW Download complete");
            } else {
                    BT_ERR("%s: No USB Device info available!", __func__);
                    return -ENODEV;
            }
            BT_ERR("=========================================\n");
#endif
            ...
            /* Before going any further, let's try to make sure that the*/
            /* state of the device is in a 'known' good state.          */
            /* * NOTE * Earlier versions used combinations of the       */
            /*          following functions: - usb_reset_endpoint() -   */
            /*          usb_clear_halt() While, in theory, this should  */
            /*          have worked, some later kernel versions did not */
            /*          clean up the endpoint states correctly.  This   */
            /*          would lead to device open errors after the      */
            /*          device was closed because the Interrupt endpoint*/
            /*          would not receive events.  Changing this to -   */
            /*          usb_reset_configuration() which is documented to*/
            /*          clean up all internal endpoint states seems to  */
            /*          work on all tested versions (including behind   */
            /*          USB Hubs which previously only worked on some   */
            /*          kernel versions).                               */
            /* * NOTE * It is believed that the: - usb_kill_urb()       */
            /*          function on the interrupt endpoint is what      */
            /*          caused the strange state for the interrupt      */
            /*          endpoint (this was - and still is - called when */
            /*          the device is closed).                          */

            usb_autopm_get_interface(USBDeviceInformationPtr->AsyncInterfacePtr);
            usb_reset_configuration(USBDeviceInformationPtr->USBDevice);

            StartHCIEventACL(USBDeviceInformationPtr, DevNum);
            usb_autopm_put_interface(USBDeviceInformationPtr->AsyncInterfacePtr);
...
static int WriteDataIoctl(USBDeviceInformation_t *USBDeviceInformationPtr, unsigned long Parameter)
{
   int                     MaxPacketSize;
   int                     BytesToSend;
   ...
   if(Parameter)
   {
      ...
      if(copy_from_user(&USBPacketInformation, (USBPacketInformation_t *)Parameter, sizeof(USBPacketInformation_t)) == 0)
      {
         switch(USBPacketInformation.PacketType)
         {
            case BTUSB_PACKET_TYPE_COMMAND:
               BT_ERR("%s: BTUSB_PACKET_TYPE_COMMAND", __func__);
               ...
               if(Parameter)
               {
                  ...
                  if(copy_from_user(&USBPacketInformation, (USBPacketInformation_t *)Parameter, sizeof(USBPacketInformation_t)) == 0)
                  {
                     ...
                     if(USBBufferInformationPtr)
                     {
                        ...
                        /* Copy the information back to Userspace memory.  */
                        ...
                        for (i = 0; i < ((ret_val > 6)?6:ret_val); i++)
                            DEBUGPRINT(KERN_INFO "0x%2x    ", USBPacketInformation.Buffer[i]);
			BT_ERR("");
                     }
                  }
                  else
               ...
               break;
            case BTUSB_IOCTL_SEND_PACKET:
               DEBUGPRINT(KERN_ERR "%s : Send Data\n", __FUNCTION__);

               ret_val = WriteDataIoctl(USBDeviceInformationPtr, Parameter);
               break;
            case BTUSB_IOCTL_ENABLE_SCO_DATA:
               ...
               if((Parameter >= BTUSB_SCO_DATA_NO_CHANNELS) && (Parameter <= BTUSB_SCO_DATA_THREE_SIXTEEN_BIT_CHANNELS))
               {
               ...
               break;
            default:
               BT_ERR("%s: Invalid IOCTL request(cmd: %d)!!!", __func__, Command);
               break;
         }
      }
      else {
         BT_ERR("%s: No USB Device information present!", __func__);
         ret_val = -ENODEV;
      }
   } else
    BT_ERR("%s: Invalid file pointer!!!", __func__);
...
#ifdef CONFIG_PM
   ContextInformation.USBDriver.suspend    = USBSuspend;
   ContextInformation.USBDriver.resume     = USBResume;
#endif
   ContextInformation.USBDriver.id_table   = BluetoothDeviceIDTable;
   ContextInformation.USBDriver.supports_autosuspend      = 1;
   ContextInformation.USBDriver.disable_hub_initiated_lpm = 1;
